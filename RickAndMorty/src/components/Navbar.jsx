...
function NavBar() {
    ...
    const pages = [ 'LOCATIONS', 'EPISODES', 'CHARACTERS', 'CONTACT'];
  ...
  return (
    ...
    <Link to="/"  >
          {theme.palette.mode !== "dark" ? (
            ...
            <img src={logowhite} alt='logo' style={{width: '6.5rem', marginLeft: '2rem'}} />  
    ...
    {pages.map((page) => (
      <Button key={page} onClick={toggleNavMenu} sx={{ my: 2, color: 'black', fontSize: '16px', fontWeight: '700' }}>
         <NavLink to={`${page.toLowerCase()}`} style={{ textDecoration: 'none', color: `${theme.palette.primary.contrastText}`}}>{page}</NavLink>
